...
import fibonnaciheap as Heapfibo
#variables globales
pilavecinos=Heapfibo.FibonacciHeap()
listavecinos=[]
listavecinosvalues=[]
list_initialvalues=[]
matriz2object=[]
            ...
            if (celda.tipo!="frozen"):
                ...
                if (enter):
                    ...
                    if celda.tipo=="open":
                        ...
                        celda.tiempo=value
                        celda.tail=i
                        pilavecinos.insert(celda)
                    elif celda.tipo=="narrow":
                        if value<celda.tiempo:
                            #agregado
                            celda.tiempo=value
                            celda.tail=i
                            pilavecinos.consolidate()
...
def main_loop():
    cont=0
    nodo=pilavecinos.extract_min()
    while (nodo):
        cont+=1
        coordenadasvecino=nodo.key.coordenadas
