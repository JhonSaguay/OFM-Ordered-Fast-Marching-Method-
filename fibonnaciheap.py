...
class FibonacciHeap:
    ...
    def insert(self, key, value=None):
        ...
        if self.min_node is None or n.key.tiempo < self.min_node.key.tiempo:
            self.min_node = n
        self.total_nodes += 1
        return n
    ...
    def decrease_key(self, x, k):
        if k.tiempo > x.key.tiempo:
            return None
        x.key = k
        y = x.parent
        if y is not None and x.key.tiempo < y.key.tiempo:
            self.cut(x, y)
            self.cascading_cut(y)
        if x.key.tiempo < self.min_node.key.tiempo:
            self.min_node = x
    ...
    def merge(self, h2):
        ...
        if h2.min_node.key.tiempo < H.min_node.key.tiempo:
    ...
    def consolidate(self):
        ...
        for i in range(0, len(A)):
            if A[i] is not None:
                if A[i].key.tiempo < self.min_node.key.tiempo:
                    self.min_node = A[i]
