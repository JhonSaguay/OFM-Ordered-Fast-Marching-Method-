    ...
    for celda in von_neuman_list:
        ...
        if (celda.velocidad!=0):
            if (celda.tipo!="frozen"):
                ...
                enter=True
                if (padding!=0):
                    if (y+1<=len(matriz2object[0])-1) and (x+1<=len(matriz2object)-1) and (y-1>=0) and (x-1>=0) :         
...
def main_loop():
    ...
    while len(listavecinos)>0:
        cont+=1
        coordenadasvecino=find_min_neighborlist()
...
#A = Timage.conversion_image('laberinto.jpg')
A = Timage.conversion_image('imagen_800x800.png')
