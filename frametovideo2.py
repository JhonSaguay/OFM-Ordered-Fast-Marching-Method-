...
def convert_frames_to_video(input_list,output_file_name,fps,size):
    ...
    for i in range(num_frames):
        base_name='Img_diagonal_'
        img_name = base_name + str(i) + '.jpg'
...
#    img_list = glob.glob(input_frame_path+'/*.png')
    #num_frames = 2000
    frame = cv2.imread(os.path.join(input_frame_path,'Img_diagonal_0.jpg'))
    height, width, channels = frame.shape
    fps = 30
    output_file_name = 'output_video/{0}_{1}fps.mp4'.format(data_dir,fps)
